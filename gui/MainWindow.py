...
class MainWindow(QWidget):
    def __init__(self, config):
        ...
        # Barre de navigation
        nav_widget = QWidget()
        nav_widget.setFixedWidth(config['style']['navbar_width'])
        nav_layout = QVBoxLayout(nav_widget)
        nav_widget.setStyleSheet(
            f"background-color: {config['style']['navbar']};")
        ...
        for i, page_class in enumerate(pages_classes):
            ...
            btn.setStyleSheet(f"""
                background-color: {config['style']['navbar_button']}; 
                color: {config['style']['button_text']}; 
                font-size: {config['style']['font_size']['button1']};
                font-weight: bold; 
                padding: 8px;
            """)
            btn.clicked.connect(
                lambda _, idx=i: self.pages.setCurrentIndex(idx))
